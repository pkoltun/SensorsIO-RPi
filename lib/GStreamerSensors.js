...
function Camera(gstreamerRoot, singleClient, logger) {
    ...
    var arg = '-e -vvv rpicamsrc annotation-mode=%annotation% bitrate=%bitrate% vflip=%vflip% hflip=%hflip% exposure-mode=%exposureMode% ! video/x-h264,width=%width%,height=%height%,framerate=%fps%/1,profile=%profile% ! rtph264pay pt=96 config-interval=1 ! udpsink host=%client% port=%port%';
	if (!singleClient){
